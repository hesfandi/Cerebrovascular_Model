function [H,BC]=Cerebrovascularmodel_non_optimzed()
N2=4;
N1=29;
%%
offset_P=0;
...
G.Edges.D=G.Edges.D;
G.Edges.L(find(G.Edges.Type==10))=L_sph;
...
K.Edges.L(a)=L_Pi;
K.Edges.Type(a)=7;
K.Edges.D(a)=D_intersect-step_pi;
K.Nodes.X(s)=K.Nodes.X(art1)-23;
K.Nodes.Y(s)=K.Nodes.Y(art1)+23;
...
for i=1:45
    ...
    K.Edges.L(a)=L_Pi;
    K.Edges.Type(a)=7;
    K.Edges.D(a)=D_intersect-step_pi*(i);
    K.Nodes.X(s)=K.Nodes.X(s-1)-23;
    K.Nodes.Y(s)=K.Nodes.Y(s-1)+23; 
    K.Nodes.Z(s)=K.Nodes.Z(s-1);
end
%%
g=K;
...
K.Edges.Type(a)=8;
K.Edges.D(a)=72;
K.Nodes.X(s)=K.Nodes.X(vein1)-23;
K.Nodes.Y(s)=K.Nodes.Y(vein1)+23;
...
for i=1:45
    ...
    K.Edges.Type(a)=8;
    K.Edges.D(a)=72-1*i;
    K.Nodes.X(s)=K.Nodes.X(s-1)-18;
    K.Nodes.Y(s)=K.Nodes.Y(s-1)+18; 
    K.Nodes.Z(s)=K.Nodes.Z(s-1);
end
% BC_Nodes=[BC_Nodes,s];
%%
...
for j=1:15
    ...
    aa=(X1+1:X1+N1-1)';
    ba=(X1+2:X1+N1)';
    X_a=repmat(1370-x_step*(j-1),N1,1);
    Y_a=repmat(0+y_step*(j-1),N1,1);
    Z_a=(linspace(0,-840,N1))';
    L_a=repmat(L_PA,length(aa),1);
    Type_a=repmat(2,length(aa),1);
    D_a=linspace(D_UU(end+1-j),D_LL(end+1-j),length(aa))';
    %%
    av=(X1+N1+1:X1+2*N1-1)';
    bv=(X1+N1+2:X1+2*N1)';
    X_v=repmat(1500-(x_step)*(j-1),N1,1);
    Y_v=repmat(200+(y_step)*(j-1),N1,1);
    Z_v=(linspace(0,-840,N1))';
    L_v=repmat(L_PA,length(av),1);
    Type_v=repmat(1,length(av),1);
    D_v=linspace(D_U1,D_L1,length(av))';
...
% Extracting the Node positions
    ...
    Y_C1=100+y_step*(j-1)-Y_C1';
    X_C1=1450-x_step*(j-1)+X_C1';
    n_nod_C=numnodes(g_C1);
    Z_C1=repmat(Z_a(4),n_nod_C,1);
    % figure
    % plot(g_C1, 'XData', X_C1, 'YData', Y_C1,'LineWidth',0.1);
...
D_UUU=[D_UU(2:end),D_UU(end:-1:1)];
D=[G.Edges.D;D_UUU';repmat(D_U1,length(NodesVein),1)];
L=[G.Edges.L;repmat(0.5,length(Nodesart),1);repmat(0.5,length(NodesVein),1)];
EdgeTable=table(EndNodes,Type,L,D);
X=G.Nodes.X;
Y=G.Nodes.Y;
Z=G.Nodes.Z;
...
K.Edges.D(find(K.Edges.Type==10))=5;
